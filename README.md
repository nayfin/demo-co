...
We are going to be working with a component library called `demo-library`. It's designed to facilitate inline editing of documents, similar to updating a single field in a JIRA ticket. Right now there is only a single component, but we'll add Storybook now. We want to make sure we are documenting these components as we build them to help ensure adoption in our organization.

When you follow this guide with your own project, substitute your project's name for `demo-library` in all of the bash commands.
...
```bash
nx g @nrwl/angular:storybook-configuration demo-library
```

Now run storybook

```bash
nx run demo-library:storybook
```
...
Then we update our story (file `libs/demo-library/src/lib/components/text/text.component.stories.ts`) by importing the `action` function from Storybook at the top of the file
...
During the Storybook setup we opted to have the plugin setup up cypress for us. So it set up an e2e project(`demo-library-e2e`) in the `apps` folder configured to run against our stories, and it stubbed out a basic test for the demo-library story that was generated.

`apps/demo-library-e2e/src/integration/text.component.ts`
...
describe('demo-library', () => {
  ...
  it('should render the component with correct background-color', () => {
    cy.get('demo-library-text').should('exist')
  });
});
...
```bash
nx run demo-library-e2e:e2e --watch
```
...
```ts
  it('should render the component with correct background-color', () => {
    cy.get('demo-library-text').should()
  });
```
...
nx g @twittwer/compodoc:config demo-library
```
...
import compodocJson from '../../../dist/compodoc/demo-library/documentation.json';
...
export default {
  ...
  title: 'demo-library Text Component',
...
```bash
nx run demo-library:compodoc:json --watch
```
...
export class TextComponent implements OnInit {
   ...
   */
  @Input() state: demo-libraryState = 'editing';
  ...
}
```
...
```bash
nx generate @nrwl/angular:stories demo-library --generateCypressSpecs
```
...
export const withTemplate: Story<TextComponent> = (args: TextComponent): IStory => ({
  ...
  template: `
    <h2>Displaying</h2>
    <demo-library-text [textValue]="'initialValue'" [state]="'displaying'"></demo-library-text>
    <h2>Editing</h2>
    <demo-library-text [textValue]="'initialValue'" [state]="'editing'"></demo-library-text>
    <h2>Updating</h2>
    <demo-library-text [textValue]="'initialValue'" [state]="'updating'"></demo-library-text>
  `
});
```
    ...
    "demo-library": {
      "architect": {
        ...
        "storydoc": {
          ...
          "options": {
            "commands": [
              "npx nx run demo-library:compodoc:json --watch",
              "npx nx run demo-library:storybook"
            ]
          }
        },
        "build-storydoc": {
          ...
          "options": {
            "commands": [
              "npx nx run demo-library:compodoc:json",
              "npx nx run demo-library:build-storybook"
            ]
          }
        }
      }
...
Now we can run `nx run demo-library:storydoc` to run in watch mode `nx run demo-library:storydoc` to build storybook for deployment to a docs or showcase page.
